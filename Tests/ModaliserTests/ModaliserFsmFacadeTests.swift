...
@Suite("(modaliser fsm) modal façade")
struct ModaliserFsmFacadeTests {
    ...
    @Test func treeRootAndConfigurationRoundtrip() throws {
        let engine = try SchemeEngine()
        try engine.evaluate("""
          (import (modaliser fsm) (modaliser dsl) (modaliser configuration))
        ...
        """)
        #expect(try engine.evaluate("(configuration-tree-ref cfg \"global\")") != .false)
    }
    ...
    // its legend block child + display panel clause) rendered as an EMPTY
    ...
    @Test func navigateToPathFallsBackToProvidedRestingStatesPayload() throws {
        let engine = try SchemeEngine()
        try engine.evaluate("(import (modaliser dsl) (modaliser fsm) (modaliser configuration))")
        try engine.evaluate(#"""
                                ...
                                (cons 'states
                                  (list (provided-state "narrow-fallback-test/a"
                                          'payload (list (cons 'children
                                                           (list (list (cons 'type 'stub-block))))
                                                         (cons 'display
                                                           '((panels . (((label . "Jump") (span . wide)
                                                                         (rows . ((block . stub-block)))))))))
                                          (edge 'up 'narrow-fallback-test))))))))))
          (fsm-install-graph! (lower-configuration cfg))
          (modal-activate! "narrow-fallback-test" '() F18)
          (define RESOLVED
            (navigate-to-path (configuration-tree-ref cfg "narrow-fallback-test") (list "a")))
        """#)
        #expect(try engine.evaluate("(not (eq? RESOLVED #f))") == .true)
        #expect(try engine.evaluate("(pair? (node-display-ref RESOLVED 'panels))") == .true)
        #expect(try engine.evaluate("(length (node-children RESOLVED))") == .fixnum(1))
        #expect(try engine.evaluate(#"""
            (equal? (cdr (assoc 'label (car (node-display-ref RESOLVED 'panels)))) "Jump")
        """#) == .true)
    ...
    @Test func navigateToPathStillReturnsFalseForAnUnknownKey() throws {
        ...
        try engine.evaluate("(import (modaliser dsl) (modaliser fsm) (modaliser configuration))")
        try engine.evaluate("""
          (define cfg (configuration (tree 'no-provider-test
            (tree-root 'no-provider-test (key "a" "A" (lambda () 'ok))))))
        """)
        #expect(try engine.evaluate(
            "(navigate-to-path (configuration-tree-ref cfg \"no-provider-test\") (list \"z\"))") == .false)
    }
}
