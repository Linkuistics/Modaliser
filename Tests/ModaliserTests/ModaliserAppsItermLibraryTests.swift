...
struct ModaliserAppsItermLibraryTests {
    ...
    // Regression: prior to this test, iterm-context-suffix-handler called
    // (iterm-rebuild-tree!) with no opts, so any 'sticky-mode-id passed to
    // iterm-register! got reverted to the default 'iterm-panes-focus on
    ...
    // didn't exist for that user. The fix captures opts in iterm-register!
    ...
    @Test func registerOptsSurviveContextSuffixRebuild() throws {
        ...
        // so iterm-register! installs a closure that should re-apply opts on every
        // dispatcher call.
        try engine.evaluate("(iterm-register! 'sticky-mode-id 'my-iterm-focus)")
    ...
    // exported (iterm-context-suffix-handler bundle-id . opts) form used by
    ...
    @Test func standaloneHandlerHonoursForwardedOpts() throws {
        ...
        try engine.evaluate("(iterm-register! 'install-context-suffix? #f 'sticky-mode-id 'manual-focus)")
        ...
        _ = try engine.evaluate("""
          (iterm-context-suffix-handler "com.googlecode.iterm2"
