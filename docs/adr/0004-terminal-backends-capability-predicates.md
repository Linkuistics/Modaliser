...
The façade's 13 ops can be `#f` (unsupported) for the active backend
(e.g. `move-pane-*` on Ghostty 1.3.1; all 13 on Alacritty when no mux
is hosted inside). Calling an unsupported op errors. To let one
generic tree gate bindings by backend support, `(modaliser terminal)`
exports a small set of capability predicates plus a universal
...
- `(supports-move-pane?)` — true if `move-pane-{left,right,up,down}`
  are all supported. Distinguishes 13/13 backends from 12/13
...
- `(supports? 'focus-pane-left)` — universal escape hatch for fine-
  grained gating; takes any of the 13 op symbols.
