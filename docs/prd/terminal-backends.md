...
| Kitty            | host, splits     | ✓ `kitty @ ls` | **13/14** *with configure-entry* (0/14 raw — IPC refused without provisioning); no zoom | — | ✓ writes `allow_remote_control yes`, `listen_on unix:/tmp/kitty-modaliser`, and `enabled_layouts splits,...` to `kitty.conf` | Full directional ops via `kitty @ --to=…` IPC. No native single-pane zoom. |
