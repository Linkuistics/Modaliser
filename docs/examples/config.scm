;; Modaliser configuration — worked example.
;;
;; A complete config you can read top-to-bottom or copy into
;; ~/.config/modaliser/config.scm. Reads as a tutorial of the bundled
;; (modaliser …) libraries and the presentation-first layout DSL: the
;; overlay is authored as a tree of SCREENS, each an implicit grid of
;; PANELS (banded cards). Tweak freely; restart Modaliser (or use the
...
;;
;; (screen 'scope panel…) registers a tree as a grid of panels. Each
...
(screen 'global
  ...
  ;; grid left-to-right, wrapping onto a new row when the overlay runs out
  ;; of width.
...
;; Layout is re-snapshotted on every overlay open, so panes added or
;; moved between presses Just Work without a separate tree rebuild step.
;;
...
;; them — it appears in this screen as "Configure iTerm"
...
(screen 'com.googlecode.iterm2
  ...
  ;; operations. Each is a 0-arg procedure, so it can be passed directly
  ;; as the `key` action. A plain (group …) is a transparent flat
  ;; drill-down row (the `›` affordance) — unchanged from before; only
  ;; the surrounding authoring moved to the layout DSL.
  (group "s" "Split"
...
(screen 'company.thebrowser.dia
  (group "t" "Tabs"
    ...
    (key "s" "Select Tab…"
         (selector 'prompt    "Dia tab…"
                   'source    dia-tab-source
                   'id-field  "id"
                   'remember  "dia-tabs"
