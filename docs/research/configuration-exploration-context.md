...
| CSS | [Base CSS](../../Sources/Modaliser/Scheme/base.css), panel-grid rules | `.panel-grid` declares `display: grid-lanes` **only** — there is no preceding `display: grid` and no `@supports` guard anywhere in the tree; the separate `[data-layout="grid"]` rule is an authored opt-in, not a fallback. Verify actual WebKit support and fallback on the supported macOS range; do not infer deployment support from source comments. |
