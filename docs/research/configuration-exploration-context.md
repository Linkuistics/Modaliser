...
| CSS | [Base CSS](../../Sources/Modaliser/Scheme/base.css), panel-grid rules | `.panel-grid` declares `display: grid-lanes` **only** — there is no preceding `display: grid` and no `@supports` guard on that rule, and no stylesheet under `Sources/` contains an `@supports` at-rule at all. State the structural fact rather than a tree-wide count of the string: the research documents now discuss `@supports`, so a repository-wide grep matches them. The separate `[data-layout="grid"]` rule is an authored opt-in, not a fallback. Verify actual WebKit support and fallback on the supported macOS range; do not infer deployment support from source comments. |
