...
- **Partly superseded by `legacy-whichkey-deletion-k13` (2026-06-24):** the "Legacy forms kept, deprecated" decision recorded below (and the `set-overlay-aspect-ratio!` / `overlay-column-count` aspect-ratio column search still framed here as governing the default list renderer) was reversed by a post-implementation **flag-day deletion**. `define-tree` / `category` / `overlay` / `which-key-block` and the aspect-ratio knob were **physically removed**; the default list renderer now flows CSS-intrinsic auto-fit columns. See the flag-day amendment on [ADR-0012](../../adr/0012-layout-dsl-surface-screen-panel-open-over-unchanged-atoms.md). This spec is retained as the dated design record.
...
- **Amended (overlay-polish, responsive-columns-k30):** CSS-intrinsic auto-fit **maximizes** the column count, which over-columns short content (three short panels → three columns where two balance better). The default is now a **JS aspect-balance** pass: after the body's DOM is built, the renderer measures and picks the column count whose grid shape is closest to a target width:height ratio (≈ 1.4), then sets `--panel-grid-cols`. An authored `'cols N` still hard-pins and skips measurement (the only override; no per-screen target knob). The loose region shares **one overlay width** with the panel grid but packs its bare rows into **its own** column count (narrower items → more columns); loose blocks (diagram, live list) stay full-width. This revives, for the panel grid, the spirit of the deleted `overlay-column-count` aspect search. CSS `auto-fit` remains the no-JS fallback track expression.
