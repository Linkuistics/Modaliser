...
Implement six features plus app icon integration to extend Modaliser with clipboard history, quicklinks/snippets selectors, overlay performance improvements, async shell commands, and a full Scheme-based state machine. The original `define-app-tree` macro is replaced by a simpler "copy bundle ID" action in the existing app finder.
...
| `define-app-tree` | Dropped | Replace with "copy bundle ID" action in app finder chooser |
...
4. Copy bundle ID action
5. Quicklinks & Snippets selectors
6. Clipboard history
7. State machine in Scheme
...
- New method `updateContent(content:theme:)` clears and rebuilds subviews within the existing container, then resizes the panel if entry count changed
- `OverlayCoordinator.modalDidNavigate()` calls `updateContent` when the panel is already visible, `showOverlay` only for first display
...
**Testing:**
- Extend `OverlayCoordinatorTests` to verify that `showOverlay` is called once on first navigation, then `updateContent` on subsequent navigations
- `MockOverlayPresenter` gains `updateContent` tracking
...
## 4. Copy Bundle ID Action in App Finder

Add a secondary action to the existing `find-installed-apps` selector that copies the selected app's bundle ID to the clipboard.

**Swift-side change:**
- Verify that the alist returned by `find-installed-apps` includes a `'bundle-id` field. If not, extend `AppLibrary`/`AppScanner` to include it.
...
(selector "a" "Apps"
  ...
  'on-select (lambda (app) (activate-app (assoc-ref app 'name)))
  'actions (list
    (action "Copy Bundle ID"
      'description "Copy bundle identifier to clipboard"
      'key 'secondary
      'run (lambda (app) (set-clipboard! (assoc-ref app 'bundle-id))))))
```

**Testing:**
- Verify `find-installed-apps` returns alists that include `'bundle-id`
- Test the action lambda in `SchemeEngine` integration tests
...
**Architecture:**
- New `QuicklinksLibrary` — NativeLibrary following existing pattern. Reads and parses the `.scm` file on each call (files are small, no caching needed). Registers `get-quicklinks`.
...
(selector "q" "Quicklinks"
  ...
  'on-select (lambda (link) (open-url (assoc-ref link 'url))))

(selector "s" "Snippets"
  'prompt "Paste snippet"
  'source (lambda () (get-snippets))
  'on-select (lambda (snip)
    (set-clipboard! (expand-snippet (assoc-ref snip 'content)))
...
- **Paste-back** — On select, restores ALL types from the entry directory to NSPasteboard, so the receiving app gets the richest format it supports.
...
(selector "v" "Clipboard"
  ...
  'on-select (lambda (entry)
    (restore-clipboard-entry! (assoc-ref entry 'id))
    (send-keystroke '(cmd) "v")))
```
...
- Unit tests for `ClipboardHistoryLibrary`: Scheme function integration
- Test that entry directories contain correct UTI files
- Test paste-back restores all types

---

## 7. State Machine in Scheme

Reimplement `ModalStateMachine` in Scheme. Swift's `KeyEventDispatcher` becomes a thin bridge that forwards key events to Scheme and acts on the returned result.
